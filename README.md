...
See `SOURCES.md` for the exact upstream URLs and license notes.

## Custom MikeTechSecurity list

Add your own domains, one per line, to:

`lists/miketechsecurity-custom.txt`
...
Keep this custom file separate from the automated mirror files because the update workflow intentionally does not overwrite it.

## Important

Pi-hole DNS filtering is not a replacement for pfSense firewall policy. Use pfSense for VLAN isolation, forced DNS, schedules and access-control rules; use Pi-hole groups for DNS-domain filtering.
